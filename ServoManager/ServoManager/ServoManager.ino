...
#define SERV_0_OPEN 444
#define SERV_0_CLOSE 111
#define SERV_1_OPEN 111
#define SERV_1_CLOSE 444
...
void loop() {
  ...
  if (serialResult == MSG_METHOD_SUCCESS)
  {
    ...
    {
      String parseString = command.substring(3,4);
      int serv = parseString.toInt();
      parseString = command.substring(4,7);
      int pos = parseString.toInt();
      pwm.setPWM(serv, 0, pos);
      //Serial.print("M2: " + String(M2));
    }
    else if (command.substring(0,4)=="open")
    {
      pwm.setPWM(0, 0, SERV_0_OPEN);
      pwm.setPWM(1, 0, SERV_1_OPEN);
    }
    else if (command.substring(0,5)=="close")
    {
      pwm.setPWM(0, 0, SERV_0_CLOSE);
      pwm.setPWM(1, 0, SERV_1_CLOSE);
    }
  }
  /*
